...
|-------|------------|-----|
| Duplicate noon run failure (Run #267) | Redirecting python stdout logs directly to `$GITHUB_ENV` broke runner parser with `Invalid format '✅ Preflight...'`. Downstream `logitech-report` ran redundantly and consumed Gemini calls. | Only append strict `NAME=VALUE` to `$GITHUB_ENV`; check delivery in `prepare` job to skip downstream jobs as clean green NO-OP. Add defense-in-depth step check in `logitech-report`. |
| Logitech VLM unsure due to empty feeder keyframes (2026-09-12) | Laplacian sharpness on bowl region selected empty feeder hardware and bowl rim (`lap=3.8`) over cat fur (`lap=1.5`), sending empty bowl frames to Gemini. | Upper-body ROI (`y: 0-60%`) with empty feeder ratio filtering and coat pattern variance scoring, ensuring genuine cat body frames are sent. |
