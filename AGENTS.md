...
| `kibble_dispensed` snapshot waits for a stable clear count, fallback to best-kibble in 5s | Take the snapshot after 3 consecutive clear frames with the same visible kibble count so the owner can inspect the kibble. If Dan never clears the bowl, use the highest-kibble frame within 5s. |
