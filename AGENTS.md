...
| `kibble_dispensed` snapshot waits through post-hand pre-arrival window | Use a stable no-cat frame before cats cover the bowl when available, so Telegram shows inspectable kibble. Do not lock the snapshot at the Dan_hand falling edge; keep collecting clean frames until a cat arrives at the bowl or timeout. If no clean pre-cat frame exists, wait for 3 consecutive clear frames after Dan_hand; final fallback is the highest-kibble frame within 5s. |
| Kibble snapshots must not depend on confirmed `Dan_hand` | If `Dan_hand` is filtered out or absent, Telegram still needs an inspectable pre-cat Kibble snapshot. Emit a generic `kibble_dispensed` snapshot at first bowl arrival from the best clean pre-cat frame. |
| Do not send `kibble_start` snapshots | First-visible Kibble frames can be noisy early frames (e.g. 1 Kibble at 06:20:06) and look identical to the broken `kibble_dispensed` symptom in Telegram. Send `kibble_dispensed_epN` instead. |
